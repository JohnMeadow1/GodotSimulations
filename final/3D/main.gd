...
func _physics_process(delta):
	...
	get_node("Ray_geometry").get_material_override().albedo_color =  Color( 1, 0, 0 ) 
	get_node("Hit"         ).get_material_override().albedo_color =  Color( 1, 0, 0 )
	...
	if ( (P2 - P1).cross(surface_Hit - P1) ).dot(surface_Normal) > 0:
		if ( (P3 - P2).cross(surface_Hit - P2) ).dot(surface_Normal) > 0:
			if ( (P1 - P3).cross(surface_Hit - P3) ).dot(surface_Normal) > 0:
				get_node("Hit"         ).get_material_override().albedo_color = Color( 0, 1, 0 ) 
				get_node("Ray_geometry").get_material_override().albedo_color = Color( 0, 1, 0 )
				Hit    = surface_Hit
...
func update_ray():
	...
	ray_geo.clear()
	ray_geo.begin( VisualServer.PRIMITIVE_LINE_STRIP, null )
...
func generate_geometry():
	...
	var target_geo = get_node( "Target_geometry" )
	target_geo.begin( VisualServer.PRIMITIVE_TRIANGLES, null )
...
func generate_geometry_wireframe():
	var wireframe_geo = get_node("Target_wireframe")
	wireframe_geo.begin( VisualServer.PRIMITIVE_LINE_STRIP, null )
