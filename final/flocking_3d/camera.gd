...
func _physics_process( delta ):
	camera_target = get_parent().get_node("buildings").get_child(target_id).get_translation()
...
#	get_parent().get_node("boids").get_child(target_id).material_override.set("albedo_color", Color(1,0,0))